...
final class ArticleRequestTest: XCTestCase {
    ...
    func test_fetchArticlesAlive() {
        ...
        ArticleRequest(requestQueryItem: Article.Request(query: "alive?")).request { result in
            switch result {
            case .success:
                exp.fulfill()
            case .failure:
    ...
    func test_fetchArticles_DecodeSuccess() {
        ...
        ArticleRequest(requestQueryItem: Article.Request(query: "decodeSuccess")).request { result in
            switch result {
